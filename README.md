...
immersive **welcome screen**: the greeting (and any choices) is centered
vertically, and the connecting/thinking state is centered where the message will
appear. The input stays hidden until the first message arrives; if the message
has choices, the input is pulled up beneath the greeting with the choices below.
It persists until the customer sends or selects something, then the conversation
continues as a normal transcript. If the first message carries modalities or a
guide, the normal transcript is used instead.

It's enabled by default. Set `welcomeScreen: false` to render a top-anchored
transcript from the first message. When a `brandIcon` is set, it appears at the
top of the welcome screen; set `welcomeScreenLogo: false` to hide it.
...
When participant info is shown (`showParticipantInfo: true`), the automated
assistant is labeled **"AI"** with a default icon. Override both to match your
brand with `assistantName` (the displayed name) and `assistantIcon` (an image URL
used as the assistant's avatar). `avatarShape` controls whether participant
avatars are `"round"` (default) or `"square"`.
...
const touchpoint = await create({
  ...
  assistantName: "Ada",
  assistantIcon: "https://example.com/assistant.png",
  avatarShape: "square",
});
```
...
const touchpoint = await create({
  ...
  input: "text",
  modalityComponents: {
    MyModality: ({ data, conversationHandler }) => {
      return html`
        <div>
          <p>Are you sure?</p>
...
Touchpoint also registers a `<connect-touchpoint>` custom element. Set its
writeable `touchpointConfiguration` property to the same object you would pass to
[`create()`](#api-reference) and it starts in **embedded mode**: no launch button,
and the UI fills the container you place it in, so it drops into an existing page
layout.
...
<div class="my-grid-layout">
  ...
  <aside>
    <connect-touchpoint></connect-touchpoint>
  </aside>
</div>
<script>
  document.querySelector("connect-touchpoint").touchpointConfiguration = {
    config: {
      // Your StartChatContact endpoint (e.g. an API Gateway route) that mints a
      // participant token.
      chatEndpoint: "REPLACE_WITH_START_CHAT_ENDPOINT",
...
The element sizes itself with low-specificity defaults — `display: block;
height: 350px` for `text` and `voice`, `display: inline-block` for `voiceMini` —
which you can override to fit your layout:
...
When the underlying Amazon Connect contact ends — the voice call is disconnected
(including after an escalated agent resolves and hangs up) or the chat contact is
closed — Touchpoint fires `onContactEnded`. For voice inputs the default action
collapses the widget back to the launcher; chat keeps the ended transcript
visible. The event is cancelable, so you can take over:

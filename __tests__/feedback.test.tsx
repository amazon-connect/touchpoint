...
describe("useFeedback", () => {
  ...
  it("should handle clickCommentButton and enter editing state", () => {
    ...
    const [, actions] = result.current;
    act(() => {
      actions.clickCommentButton("url2", "How did we do?");
    });
    const [state] = result.current;
  ...
  it("should handle editCommentText and update text", () => {
    const { result } = renderHook(() => useFeedback(mockHandler));
    act(() => {
      result.current[1].clickCommentButton("url3", "How did we do?");
    });
    act(() => {
  ...
  it("should handle submitComment and update state on success", async () => {
    const { result } = renderHook(() => useFeedback(mockHandler));
    act(() => {
      result.current[1].clickCommentButton("url4", "How did we do?");
    });
    act(() => {
  ...
  it("should handle cancelComment and return to idle", () => {
    ...
    const [, actions] = result.current;
    act(() => {
      actions.clickCommentButton("url5", "How did we do?");
  ...
  it("should handle submission error", async () => {
    ...
    act(() => {
      result.current[1].clickCommentButton("url6", "How did we do?");

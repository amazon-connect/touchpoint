...
export const FullscreenVoice: FC<Props> = ({
  ...
  // The voice session has ended (call disconnected / escalated agent hung up).
  const terminated =
    voice != null &&
    voice !== "loading" &&
    voice.type === "success" &&
  ...
  // Report it to the app (fires onContactEnded, collapses by default).
  // useLayoutEffect runs before paint, so a following collapse doesn't flash the
  // ended screen below.
